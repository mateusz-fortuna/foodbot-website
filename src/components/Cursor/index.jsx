...
export class Cursor extends Component {
  ...
  componentDidMount() {
    ...
    if (this.props.reference.flat(Infinity)[0] !== undefined) {
      this.initHovers();
      this.setState({ test: this.props.reference });
    }
  }
  ...
  initCursor() {
    ...
    this.cursorHandler = (event) => {
      this.group.position = new paper.Point(event.clientX, event.clientY);
      this.canvasCursorSpeedTimeout = setTimeout(() => {
  ...
  initCanvas() {
    ...
    const strokeColor = 'rgba( 14, 14, 14, 0.5 )';
    const strokeWidth = 1;
    const segments = 8;
    const radius = 15;
      ...
      if (isStuck && polygon.bounds.width < shapeBounds.width) {
        ...
        if (isNoisy) {
          polygon.segments.forEach((segment, index) => {
            segment.point.set(bigCoordinates[index][0], bigCoordinates[index][1]);
          });
    ...
    return (
      <div className="cursorContainer--outer">
        <div className="cursorContainer--inner">
          <div ref={this.smallCursor} className="cursor cursor--small" />
          <canvas ref={this.canvas} className="cursor cursor--canvas" data-paper-resize />
        </div>
      </div>
    );
  }
